...
We can declare a 2-dimensional array with:

=int a[5][10];=

This is an array with 5 rows with 10 integers each (i.e. 10 columns).  However what if we want to allow each row to have a different number of integers?
